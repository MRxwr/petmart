...
if ( isset($_POST["enTitle"]) && !isset($_POST["edit"]) ){
	...
	?>
	<script>
		window.location.href = "?page=brands";
	</script>
	<?php
}
...
<div class="pull-left">
<h6 class="panel-title txt-dark">Brand Details</h6>
</div>
...
<form action="?page=brands&id=<?php echo $_GET["id"] ?>" method="post" enctype="multipart/form-data">
...
if ( !isset($_GET["edit"]) ){
$status 		= array('0','1');
$arrayOfTitles 	= array('Active brands','Inactive brands');
...
<a href="?page=brands&edit=<?php echo $row["id"] ?>&id=<?php echo $row["vendorId"] ?>" style="margin:3px"><i class="fa fa-edit"></i></a>

<a href="?page=brands&id=<?php echo $_GET["id"] ?>&<?php echo $action[$i] . $row["id"] ?>" style="margin:3px"><i class="<?php echo $icon[$i] ?>"></i></a>
