...
<div class="pull-left">
<h6 class="panel-title txt-dark">Post Details</h6>
</div>
...
<form action="?page=items<?php if ( isset($_GET["id"]) ){echo "&id=".$_GET["id"];} ?>" method="post" enctype="multipart/form-data">
...
<div class="input-group">
<div class="input-group-addon"><i class="fa fa-text-width"></i></div>
<input type="text" class="form-control" id="" placeholder="Enter Arabic Details" name="age" <?php if(isset($_GET["edit"])){?>value="<?php echo $data[0]["age"] ?>"<?php }?> required>
</div>
</div>
</div>	
...
<div class="col-md-4" id="phide" style="<?php echo $styleHidden ?>">
<div class="form-group">
...
if ( !isset($_GET["edit"]) ){
$status 		= array('0','1');
$arrayOfTitles 	= array('Active Posts','Inactive Posts');
...
while ( $row = $result->fetch_assoc() ){
	$customer = selectDB("customers","`id` = '{$row["customerId"]}'");
	$category = selectDB("categories","`id` = '{$row["categoryId"]}'");
	$type = selectDB("tags","`id` = '{$row["type"]}'");
	if ( $row["ageType"] == 0 ){
		$ageType = "Day";
	}elseif( $row["ageType"] == 1 ){
		$ageType = "Week";
	}elseif( $row["ageType"] == 1 ){
		$ageType = "Month";
	}else{
...
<td><?php echo $row["age"] . " " . $ageType?></td>
<td><?php echo $customer[0]["name"] ?></td>
<td><a href="tel:<?php echo $customer[0]["mobile"] ?>">Call</a></td>
<td><a href="mailto:<?php echo $customer[0]["email"] ?>">Email</a></td>
<td><?php echo $type[0]["enTitle"] ?></a></td>
<td><?php echo $category[0]["enTitle"] ?></td>
<td><?php echo $price = ( $row["type"] != 1 ) ? 0 : $row["price"] ?>KD</td>
...
<a href="?page=images&id=<?php echo $row["id"] ?>" style="margin:3px"><i class="fa fa-file-image-o"></i></a>
