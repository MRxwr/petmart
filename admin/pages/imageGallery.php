...
if( isset($_FILES['logo']) AND is_uploaded_file($_FILES['logo']['tmp_name']) ){
	...
	?>
	<script>
	window.location.href = "?page=imageGallery&id=<?php echo $_GET['id'] ?>";
	</script>
	<?php
}
...
<a href="?page=imageGallery&id=<?php echo $_GET["id"] ?>&<?php echo $action[$i] . $row["id"] ?>" style="margin:3px"><i class="<?php echo $icon[$i] ?>"></i></a>
