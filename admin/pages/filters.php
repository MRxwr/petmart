...
if ( isset($_POST["enTitle"]) && !isset($_POST["edit"]) ){
	...
	?>
	<script>
		window.location.href = "?page=filters";
	</script>
	<?php
}
...
<div class="pull-left">
<h6 class="panel-title txt-dark">Filter Details</h6>
</div>
...
<form action="?page=filters&id=<?php if ( isset( $_GET["id"]) ){ echo $_GET["id"];} ?>" method="post" enctype="multipart/form-data">
...
if ( !isset($_GET["edit"]) ){
$status 		= array('0','1');
$arrayOfTitles 	= array('Active filters','Inactive filters');
...
<a href="?page=filters&edit=<?php echo $row["id"] ?>&id=<?php echo $row["id"] ?>" style="margin:3px"><i class="fa fa-edit"></i></a>

<a href="?page=filters&id=<?php echo $_GET["id"] ?>&<?php echo $action[$i] . $row["id"] ?>" style="margin:3px"><i class="<?php echo $icon[$i] ?>"></i></a>
