...
if ( isset($_POST["enTitle"]) && !isset($_POST["edit"]) ){
	...
	?>
	<script>
		window.location.href = "?page=categories&id=<?php echo $_GET['id'] ?>";
	</script>
	<?php
}
...
<form action="?page=categories&id=<?php echo $_GET["id"] ?>" method="post" enctype="multipart/form-data">
...
<a href="?page=categories&edit=<?php echo $row["id"] ?>&id=<?php echo $row["vendorId"] ?>" style="margin:3px"><i class="fa fa-edit"></i></a>

<a href="?page=items&id=<?php echo $row["id"] ?>" style="margin:3px"><i class="fa fa-reorder "></i></a>

<a href="?page=categories&id=<?php echo $_GET["id"] ?>&<?php echo $action[$i] . $row["id"] ?>" style="margin:3px"><i class="<?php echo $icon[$i] ?>"></i></a>
