...
if (isset($_POST["apartment"]) AND !empty($_POST["apartment"])){
	$place = $_POST["place"];
	if ( $_POST["country"] == "KW" ){
...
{
$sql = "SELECT
		p.*, i.imageurl , sp.price AS subPrice, sp.size, sp.sizeAr, sp.color, sp.colorEn, p.freeDeliv
		FROM `products` AS p
		JOIN `images` AS i
		ON p.id = i.productId
...
<div class="checkoutsidebar-item">
    ...
    <?php 
        ...
        {
            echo $price2 = $row["subPrice"] - ( $userDiscount * $row["subPrice"] / 100 );
        } ?>KD
    </span>
</div>
<?php
...
<input type="submit" name="submit" class="btn btn-large" style="width:100%; color: white; background-color:#185b99;" value="<?php echo $proceedToPaymentText ?>">
</form>
</div>
